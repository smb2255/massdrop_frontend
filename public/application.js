...
var config = {
  apiOrigins: {
    production: 'https://dry-fjord-34790.herokuapp.com'
  }
};
...
var onAddJobSubmit = function onAddJobSubmit(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.addJob(data).then(ui.onAddJobSuccess).catch(ui.onAddJobFailure);
};
var onShowJobs = function onShowJobs(event) {
  event.preventDefault();
  console.log(onShowJobs);
  api.showJobs()
  // .then((data) => console.log('data from API', data))
  .then(ui.onShowJobsSuccess).catch(ui.onShowJobsFailure);
};
...
var showJobs = function showJobs() {
  return $.ajax({
    url: config.apiOrigin + '/jobs',
    method: 'GET',
    headers: {
...
var onAddJobSuccess = function onAddJobSuccess(data) {
  console.log(data);
  $('#jobID').html(data.job.url + ' id: ' + data.job._id);
};

var onAddJobFailure = function onAddJobFailure() {
  $('#jobID').text('Job not added');
};

var onShowJobsSuccess = function onShowJobsSuccess(data) {
...
var onShowJobFailure = function onShowJobFailure(data) {
  $('#showAlljobs').text('error');
};

module.exports = {
  onAddJobSuccess: onAddJobSuccess,
  onAddJobFailure: onAddJobFailure,
  onShowJobsSuccess: onShowJobsSuccess,
  onShowJobFailure: onShowJobFailure
